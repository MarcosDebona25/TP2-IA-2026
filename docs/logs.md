...
| `llm_start` / `llm_end` | **Aún no aparecen** — saldrán cuando se conecten LLMs reales (prompts, respuestas, tokens) |
| `tool_start` / `tool_end` | **Aún no aparecen** — saldrán cuando existan las tools reales (input/output) |
...
3. Mientras los agentes sean **stubs**, solo ves `node_start` y `routing` (no llaman al LLM
   ni a tools). El handler ya está listo para registrar `llm_*` y `tool_*` en cuanto existan,
   sin tocar el módulo.
