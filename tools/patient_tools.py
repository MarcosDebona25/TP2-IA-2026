...
# -------------------------------------------------------------------
# Stubs de MongoDB para el Agente Clínico (contrato provisional con B)
# -------------------------------------------------------------------

def get_patient_history(patient_id: str) -> str:
    """Devuelve el historial clínico de sesiones previas del paciente desde MongoDB.

    Retorna un resumen de las consultas anteriores en lenguaje natural.
    """
    history_db = {
        "P001": (
            "Historial del Paciente P001:\n"
            "- Sesión 2026-04-15: HbA1c = 5.4%, Glucosa en ayunas = 92 mg/dL. Paciente controlado. Sin cambios en el tratamiento.\n"
            "- Sesión 2026-05-15: HbA1c = 5.5%, Glucosa en ayunas = 95 mg/dL. Paciente controlado. Metformina 850mg c/12h."
        ),
        "P002": (
            "Historial del Paciente P002:\n"
            "- Sesión 2026-04-15: HbA1c = 6.2%, Glucosa en ayunas = 110 mg/dL. Se inicia Metformina 1000mg c/12h.\n"
            "- Sesión 2026-05-15: HbA1c = 6.8%, Glucosa en ayunas = 135 mg/dL. Glucemia en aumento. Se agrega Empagliflozina 10mg c/24h."
        ),
        "P003": (
            "Historial del Paciente P003:\n"
            "- Sesión 2026-04-15: HbA1c = 6.0%, Glucosa en ayunas = 105 mg/dL. Paciente en rango, Metformina 500mg c/12h.\n"
            "- Sesión 2026-05-15: HbA1c = 5.9%, Glucosa en ayunas = 98 mg/dL. Buen control general."
        ),
        "P004": (
            "No hay sesiones anteriores registradas en la base de datos MongoDB para el paciente P004."
        )
    }
    return history_db.get(patient_id, f"No hay sesiones anteriores registradas en la base de datos MongoDB para el paciente {patient_id}.")


def compare_with_previous_sessions(patient_id: str) -> str:
    """Compara métricas de la sesión actual del paciente con las previas registradas.

    Retorna un análisis comparativo longitudinal.
    """
    compare_db = {
        "P001": (
            "Comparación Longitudinal P001:\n"
            "- HbA1c estable en torno a 5.5% (cambio neto de +0.1% respecto al mes anterior).\n"
            "- Glucosa en ayunas estable (cambio neto de +3 mg/dL).\n"
            "- Sin alertas metabólicas previas ni actuales."
        ),
        "P002": (
            "Comparación Longitudinal P002:\n"
            "- HbA1c en aumento sostenido: de 6.2% a 6.8% en la sesión anterior, y actualmente en 7.8% (cambio de +1.0% respecto a la sesión anterior).\n"
            "- Glucosa en ayunas en aumento progresivo: de 110 mg/dL a 135 mg/dL, alcanzando actualmente 165 mg/dL.\n"
            "- Se evidencia falla al tratamiento combinado actual (Metformina + Empagliflozina) con necesidad urgente de intensificación terapéutica."
        ),
        "P003": (
            "Comparación Longitudinal P003:\n"
            "- Glucemia estable en general, pero se detecta un episodio agudo de hipoglucemia severa (55 mg/dL) en el mes actual que no estaba presente en los meses previos (donde el mínimo era 98 mg/dL).\n"
            "- La HbA1c se mantiene estable (5.9%).\n"
            "- Requiere revisar hábitos alimenticios y horario de administración de Metformina para evitar nuevos episodios de hipoglucemia."
        ),
        "P004": (
            "No se puede realizar una comparación longitudinal para P004 porque esta es la primera sesión registrada del paciente."
        )
    }
    return compare_db.get(patient_id, f"No se puede realizar una comparación longitudinal porque es la primera sesión registrada para el paciente {patient_id}.")
